...
- Keep secrets encrypted with SOPS+age. Required key path: `$HOME/.config/sops/age/keys.txt`.
- ArgoCD tracks this repository (`main`/`HEAD`) and auto-syncs with prune + self-heal.
- New app pattern: add chart under `k8s/charts/<app>/` and corresponding ArgoCD app in `k8s/apps/<app>.yaml`.
